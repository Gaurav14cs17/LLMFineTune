"""
Direct Preference Optimization (DPO) Tab - Run torchtune DPO recipes via the UI.

Align model outputs with human preferences without a separate reward model.
"""
...
class DPOTab(QWidget):
    """Direct Preference Optimization via torchtune."""
    ...
    def _build_info_banner(self):
        frame = QFrame()
        frame.setStyleSheet(
            "QFrame{background:#fff8e1;border:1px solid #ffca28;"
            "border-radius:6px;padding:10px 14px;}")
        lay = QVBoxLayout(frame)
        lay.setSpacing(4)
        title = QLabel("Direct Preference Optimization (DPO)")
        title.setStyleSheet("font-size:14px;font-weight:700;color:#f57f17;")
        lay.addWidget(title)
        desc = QLabel(
            "Train models using Direct Preference Optimization (DPO). DPO aligns "
            "model outputs with human preferences without needing a separate reward model.\n\n"
            "Torchtune supports LoRA DPO (single and multi-GPU) and Full DPO (multi-GPU). "
            "Select a recipe and config, set the checkpoint path, and add any config overrides.")
        desc.setStyleSheet("font-size:12px;color:#e65100;")
    ...
    def _build_recipe_group(self):
        ...
        if self.recipe_combo.count() == 0:
            self.recipe_combo.addItem("lora_dpo_single_device",
                                       "lora_dpo_single_device")
    ...
    def _build_paths_group(self):
        ...
        self.ckpt_dir_edit.setPlaceholderText("Path to base model checkpoints")
        ...
        self.output_dir_edit = QLineEdit("output/dpo_training")
    ...
    def _build_overrides_group(self):
        ...
        lay = QVBoxLayout(g)
        hint = QLabel(
            "Override config values. Examples: epochs=3, beta=0.1, optimizer.lr=1e-5")
    ...
    def _build_summary_card(self):
        ...
        self.summary_frame.setStyleSheet(
            "QFrame{background:#fff8e1;"
            "border:1px solid #ffca28;border-radius:4px;padding:8px 14px;}")
        lay = QHBoxLayout(self.summary_frame)
        lay.setContentsMargins(10, 6, 10, 6)
        self.summary_label = QLabel("")
        self.summary_label.setStyleSheet("font-size:13px;color:#f57f17;")
    ...
    def _build_buttons(self):
        lay = QHBoxLayout()
        self.start_btn = QPushButton("START DPO TRAINING")
        self.start_btn.setMinimumHeight(48)
        self.start_btn.setStyleSheet(
            BTN_PRIMARY_LARGE
            .replace("#394867", "#f57f17")
            .replace("#212d40", "#e65100")
            .replace("#14213d", "#bf360c"))
    ...
    def _build_log(self):
        ...
        self.log_edit.setStyleSheet("""
            QPlainTextEdit { background-color:#1a1a2e; color:#ffca28;
    ...
    def start_training(self):
        ...
        self.log_edit.appendPlainText("  Direct Preference Optimization (DPO)")
        ...
        self.worker = DPOWorker(cmd, self.project_root)
    ...
    def _on_finished(self, code):
        ...
        if code == 0:
            ...
            QMessageBox.information(self, "Success",
                "DPO training completed!\n\n"
                "Artifacts saved to the output directory.")
        else:
