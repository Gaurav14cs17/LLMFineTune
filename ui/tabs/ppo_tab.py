"""
PPO Tab - Run torchtune Proximal Policy Optimization (RLHF) recipes via the UI.

Single-device PPO with full fine-tuning.
"""
...
class PPOTab(QWidget):
    """Proximal Policy Optimization (PPO) / RLHF via torchtune."""
    ...
    def _build_info_banner(self):
        frame = QFrame()
        frame.setStyleSheet(
            "QFrame{background:#e0f2f1;border:1px solid #80cbc4;"
            "border-radius:6px;padding:10px 14px;}")
        lay = QVBoxLayout(frame)
        lay.setSpacing(4)
        title = QLabel("Proximal Policy Optimization (PPO)")
        title.setStyleSheet("font-size:14px;font-weight:700;color:#00695c;")
        lay.addWidget(title)
        desc = QLabel(
            "Train models using Proximal Policy Optimization (PPO) with RLHF. "
            "PPO uses a reward model to align LLM outputs with human preferences "
            "through reinforcement learning.\n\n"
            "Currently supports single-device full fine-tuning only. "
            "Select a config, set the checkpoint path, and add any config overrides.")
        desc.setStyleSheet("font-size:12px;color:#004d40;")
    ...
    def _build_recipe_group(self):
        ...
        if self.recipe_combo.count() == 0:
            self.recipe_combo.addItem(
                "ppo_full_finetune_single_device",
                "ppo_full_finetune_single_device")
    ...
    def _build_paths_group(self):
        ...
        self.checkpoint_dir_edit.setPlaceholderText("Path to model checkpoints")
        self.checkpoint_dir_edit.setStyleSheet(EDIT_STYLE)
        lay.addWidget(self.checkpoint_dir_edit, 0, 1)
        btn1 = QPushButton("Browse")
        btn1.setFixedWidth(80)
        btn1.setStyleSheet(BTN_SECONDARY)
        btn1.clicked.connect(lambda: self._browse_dir(self.checkpoint_dir_edit))
        lay.addWidget(btn1, 0, 2)

        lay.addWidget(QLabel("Output Dir:"), 1, 0)
        self.output_dir_edit = QLineEdit("output/ppo_training")
    ...
    def _build_overrides_group(self):
        ...
        lay = QVBoxLayout(g)
        hint = QLabel(
            "Override config values. Examples: epochs=3, batch_size=1, "
            "optimizer.lr=1e-6")
    ...
    def _build_summary_card(self):
        ...
        self.summary_frame.setStyleSheet(
            "QFrame{background:#e0f2f1;"
            "border:1px solid #80cbc4;border-radius:4px;padding:8px 14px;}")
        lay = QHBoxLayout(self.summary_frame)
        lay.setContentsMargins(10, 6, 10, 6)
        self.summary_label = QLabel("")
        self.summary_label.setStyleSheet("font-size:13px;color:#00695c;")
    ...
    def _build_buttons(self):
        lay = QHBoxLayout()
        self.start_btn = QPushButton("START PPO TRAINING")
        self.start_btn.setMinimumHeight(48)
        self.start_btn.setStyleSheet(
            BTN_PRIMARY_LARGE
            .replace("#394867", "#00695c")
            .replace("#212d40", "#004d40")
            .replace("#14213d", "#003d33"))
    ...
    def _build_log(self):
        ...
        self.log_edit.setStyleSheet("""
            QPlainTextEdit { background-color:#1a1a2e; color:#80cbc4;
    ...
    def start_training(self):
        ...
        ckpt_dir = self.checkpoint_dir_edit.text().strip()
        if ckpt_dir:
        ...
        self.log_edit.appendPlainText("  PPO Training")
        ...
        self.worker = PPOWorker(cmd, self.project_root)
    ...
    def _on_finished(self, code):
        ...
        if code == 0:
            ...
            QMessageBox.information(self, "Success",
                "PPO training completed!\n\n"
                "Checkpoints are in the output directory.")
        else:
